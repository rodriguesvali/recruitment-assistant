...
| QA-004 | Low | Local dev integration | Default frontend port `4200` was occupied during QA. The fallback port `4300` was not CORS-whitelisted by the backend, so a browser served from `4300` would not be able to call the live API and would fall back to demo mode. | Fixed on 2026-05-18. Backend CORS now allows documented alternate frontend dev origins `http://localhost:4300` and `http://127.0.0.1:4300`. Verified with focused backend CORS preflight coverage in `backend/tests/test_api_smoke.py`. | Keep README-documented alternate frontend dev ports aligned with backend CORS. | Fixed |

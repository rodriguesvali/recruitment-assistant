...
- Use environment variable `AAMAD_TARGET_RUNTIME` with values:
    ...
    - `claude-agent-sdk`
    - `cursor-sdk`
- Runtime adapter rules are loaded from `.claude/rules/adapter-<name>.md`; selected runtime should have a matching adapter file.
...
## Usage
- Operators set `AAMAD_TARGET_RUNTIME` before Build-phase implementation work; personas align generated runtime code and docs to `.claude/rules/adapter-${AAMAD_TARGET_RUNTIME}.md`.